...
const HangarDoorReveal = React.forwardRef<HTMLDivElement, HangarDoorRevealProps>(
    ({ children, className, doorScrollHeight = 1, ...props }, ref) => {
        ...
        // Kapı bir kez açıldı mı takip et
        const [doorOpened, setDoorOpened] = React.useState(false)

        // scrollYProgress değiştiğinde kapının açılıp açılmadığını kontrol et
        React.useEffect(() => {
            const unsubscribe = scrollYProgress.on("change", (value) => {
                if (value >= doorOpenProgress && !doorOpened) {
                    setDoorOpened(true)
                }
            })
            return () => unsubscribe()
        }, [scrollYProgress, doorOpenProgress, doorOpened])
        ...
        return (
            ...
            >
                ...
                {/* HANGAR KAPISI - İçeriğin üstünde, yukarı kayar. Bir kez açıldıktan sonra gösterilmez */}
                {!doorOpened && (
                    <motion.div
                        style={{ y: doorY }}
